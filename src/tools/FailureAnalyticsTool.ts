...
interface FailureAnalyticsInput {
  group_by?: string;
  time_range?: string;
...
class FailureAnalyticsTool extends MCPTool<FailureAnalyticsInput> {
  ...
  async execute(input: FailureAnalyticsInput) {
    ...
    const filteredFailures = Object.values(failures).filter((failure: any) => {
    ...
    // Return analytics data directly - MCP framework will wrap it
    return {
  ...
  private groupFailures(failures: any[], groupBy: string): FailureGroup[] {
  ...
  private determineErrorType(failure: any): string {
    // Extract the error type from the error message
    const errorMatch = failure.error_message.match(/^([A-Za-z]+Error|AssertionError|Exception):/);
    return errorMatch ? errorMatch[1] : 'Unknown Error';
  }
  
  private generateHypothesis(failure: any): string {
  ...
  private generateInsights(groups: FailureGroup[]): any {
  ...
  private calculatePriority(group: FailureGroup): number {
  ...
  private generateTriageAction(group: FailureGroup): string {
  ...
  private saveFailureGroups(groups: FailureGroup[]): void {
    try {
      // Load existing groups
      let existingGroups: Record<string, FailureGroup> = {};
