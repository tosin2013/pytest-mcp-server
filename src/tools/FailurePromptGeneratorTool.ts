...
class FailurePromptGeneratorTool extends MCPTool<PromptGeneratorInput> {
  ...
  private async generateGroupPrompt(groupId: string, style: string) {
...
4. Suggest verification steps`;
  ...
  private async generateSingleFailurePrompt(failureId: string, style: string) {
    // Load the failure
...
3. Steps to verify the fix works`;
  ...
  private async loadFailureGroup(groupId: string): Promise<FailureGroup | null> {
    try {
  ...
  private async loadFailure(failureId: string): Promise<any | null> {
    try {
  ...
  private async loadFailuresInGroup(group: FailureGroup): Promise<any[]> {
    try {
      if (fs.existsSync(FAILURES_FILE)) {
        const failuresData = fs.readFileSync(FAILURES_FILE, 'utf-8');
  ...
  private generateDetailedPrompt(group: FailureGroup, failures: any[]): string {
...
Please analyze these failures and identify the common root cause. Then suggest the most effective way to fix the issue that addresses all failures in this group.`;
  ...
  private generateConcisePrompt(group: FailureGroup, failures: any[]): string {
...
Identify the common root cause and suggest a fix.`;
  ...
  private generateStepByStepPrompt(group: FailureGroup, failures: any[]): string {
...
6. Propose verification steps`;
  ...
  private generateRootCausePrompt(group: FailureGroup, failures: any[]): string {
...
- Could there be environmental factors (dependencies, configuration)?`;
  ...
  private generateDetailedSinglePrompt(failure: any): string {
...
3. Recommend how to verify the fix works correctly`;
  ...
  private generateConciseSinglePrompt(failure: any): string {
    return `## Test Failure: ${failure.test_name}
...
Briefly analyze the root cause and suggest a fix.`;
  ...
  private generateRootCauseSinglePrompt(failure: any): string {
...
${JSON.stringify(Object.fromEntries(
  Object.entries(failure.locals).slice(0, 5)
), null, 2)}
\`\`\`
` : ''}
...
Focus exclusively on determining the root cause of this failure.`;
  ...
  private extractKeyTracebackLines(traceback: string): string {
